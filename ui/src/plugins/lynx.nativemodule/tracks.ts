...
export class LynxNativeModuleTrack extends LynxBaseTrack<NativeModuleItem[]> {
  ...
  private readonly DASHED_AREA_COLOR = new HSLColor([0, 0, 95]);
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (selected) {
        ...
        if (slice.callbackStartTs > slice.invokeEndTs) {
          ...
          const otherThreadDurationPx =
            ctx.timescale.durationToPx(otherThreadDuration);
          ctx.ctx.fillStyle = this.DASHED_AREA_COLOR.cssString;
          ctx.ctx.fillRect(timePx, y, otherThreadDurationPx, SLICE_HEIGHT);
