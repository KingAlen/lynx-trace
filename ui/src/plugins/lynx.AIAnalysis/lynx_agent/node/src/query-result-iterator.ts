...
const QUERY_CELL_INVALID_FIELD_ID = 0;
    ...
    this.totalRows = batches.reduce((total, batch) => {
      ...
      if (Array.isArray(batch)) {
        ...
        if (batch.length > 0) {
          ...
          const varintCount = cellBatch.varintCells ? cellBatch.varintCells.length : 0;
          const stringCount = cellBatch.stringCells ? cellBatch.stringCells.split('\0').filter((s: string) => s).length : 0;
          const float64Count = cellBatch.float64Cells ? cellBatch.float64Cells.length : 0;
          const blobCount = cellBatch.blobCells ? cellBatch.blobCells.length : 0;
          ...
          for (let i = 0; i < cells.length; i += columnNames.length) {
            let canFormRow = true;
            for (let j = 0; j < columnNames.length && i + j < cells.length; j++) {
              const cellType = cells[i + j];
              if (cellType === 'CELL_VARINT' || cellType === QUERY_CELL_VARINT_FIELD_ID) {
                if (varintIndex >= varintCount) canFormRow = false;
                else varintIndex++;
              } else if (cellType === 'CELL_STRING' || cellType === QUERY_CELL_STRING_FIELD_ID) {
                if (stringIndex >= stringCount) canFormRow = false;
                else stringIndex++;
              } else if (cellType === 'CELL_FLOAT64' || cellType === QUERY_CELL_FLOAT64_FIELD_ID) {
                if (float64Index >= float64Count) canFormRow = false;
                else float64Index++;
              } else if (cellType === 'CELL_BLOB' || cellType === QUERY_CELL_BLOB_FIELD_ID) {
      ...
      next(): IteratorResult<Row> {
        ...
        for (let colIndex = 0; colIndex < columnNames.length; colIndex++) {
          ...
          if (Array.isArray(currentBatch)) {
              ...
              if (cellIndex < cells.length) {
                ...
                for (let i = 0; i < cellIndex; i++) {
                  ...
                  if (prevCellType === 'CELL_VARINT' || prevCellType === 2) {
                    varintIndex++;
                  } else if (prevCellType === 'CELL_STRING' || prevCellType === 4) {
                    stringIndex++;
                  } else if (prevCellType === 'CELL_FLOAT64' || prevCellType === 3) {
                    float64Index++;
                  } else if (prevCellType === 'CELL_BLOB' || prevCellType === 5) {
                    blobIndex++;
                  }
                }
                
                if (cellType === 'CELL_VARINT' || cellType === 2) {
                  if (cellBatch.varintCells && varintIndex < cellBatch.varintCells.length) {
                    const longValue = cellBatch.varintCells[varintIndex];
                    // Handle Long objects (from protobuf)
                    if (typeof longValue === 'object' && longValue.low !== undefined) {
                      rowData[columnName] = longValue.low + (longValue.high * 0x100000000);
                    } else {
                  ...
                  if (cellBatch.stringCells) {
                    const stringValues = cellBatch.stringCells.split('\0').filter((s: string) => s);
                  ...
                  if (cellBatch.float64Cells && float64Index < cellBatch.float64Cells.length) {
                    rowData[columnName] = cellBatch.float64Cells[float64Index];
                  } else {
                    rowData[columnName] = null;
                  }
                } else if (cellType === 'CELL_BLOB' || cellType === 5) {
                  if (cellBatch.blobCells && blobIndex < cellBatch.blobCells.length) {
            ...
            if (cellBatch && cellBatch.values && rowIndex < cellBatch.values.length) {
              const cellValue = cellBatch.values[rowIndex];
              rowData[columnName] = QueryResultIterator.extractCellValue(cellValue);
            } else {
              rowData[columnName] = null;
            }
          }
