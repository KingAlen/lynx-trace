...
base::Status TrackEventTokenizer::HandleExtraArgsValues(
  ...
  for (auto it = event.debug_annotations(); it; ++it) {
    ...
    if (annotation.has_name()) {
      ...
      if (IsLynxUpdateEvent(event_name) && !pipeline_id.empty() &&
          event.has_flow_ids()) {
...
bool TrackEventTokenizer::IsLynxUpdateEvent(std::string& event_name) {
  return event_name == "FiberFlushElementTreeEnd" ||
         event_name == "TemplateAssembler::CallLepusMethod" ||
